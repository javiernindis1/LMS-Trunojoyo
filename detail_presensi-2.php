    ...
    <div class="row">
        ...
        <div class="col-md-7 col-lg-8">
            <div class="d-flex justify-content-center border-bottom mb-4 w-100">
                <a href="detail_presensi.php?kelas_id=<?= $kelasId ?>&pertemuan_id=<?= $pertemuanId ?>" class="text-decoration-none text-secondary pb-2 fw-semibold text-center w-50" style=" font-size: 14px;">Form Absensi</a>
                <a href="detail_presensi-2.php?kelas_id=<?= $kelasId ?>&pertemuan_id=<?= $pertemuanId ?>" class="text-decoration-none text-primary pb-2 fw-semibold text-center w-50" style="border-bottom: 2px solid #0d6efd; font-size: 14px;">Mahasiswa</a>
            </div>
            

            <?php
            // Ambil data mahasiswa dari data.php
            $mahasiswaList = getMahasiswaByKelas($kelasId);
            $options = ['M' => 'm', 'A' => 'a', 'S' => 's', 'D' => 'd', 'I' => 'i'];
            ?>

            <form>
                <div class="student-list-container mb-4">
                    <div class="student-list">
                        <?php foreach ($mahasiswaList as $mhs): ?>
                        <div class="student-item">
                            <!-- Kolom kiri: NIM - Nama (truncate ellipsis) -->
                            <div class="student-info-col" title="<?= htmlspecialchars($mhs['nim'] . ' - ' . $mhs['nama']) ?>">
                                <?= htmlspecialchars($mhs['nim']) ?> - <?= htmlspecialchars($mhs['nama']) ?>
                            </div>
                            <!-- Kolom kanan: tombol status M A S D I -->
                            <div class="student-status-col">
                                <?php foreach ($options as $val => $class):
                                    $id      = 'status_' . $mhs['id'] . '_' . $val;
                                    $name    = 'status[' . $mhs['id'] . ']';
                                    $checked = ($mhs['status'] === $val) ? 'checked' : '';
                                ?>
                                <input type="radio" name="<?= $name ?>" id="<?= $id ?>" value="<?= $val ?>" class="status-radio" <?= $checked ?>>
                                <label for="<?= $id ?>" class="status-label <?= $class ?>"><?= $val ?></label>
                                <?php endforeach; ?>
                            </div>
                        </div>
                        <?php endforeach; ?>
